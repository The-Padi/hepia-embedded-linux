...
void zone_clear(spi_display_t *disp, uint16_t color, int start_x, int start_y, int end_x, int end_y)
{
    ...
    uint8_t dat_fast[((end_x-start_x) * (end_y-start_y)) * 2];

    int k = 0;

    disp_setwindow(disp, start_x, start_y, end_x, end_y);
...
void w_letter(spi_display_t *disp, char let, int start_x, int start_y, int height, int width, int R_char, int G_char, int B_char, int R_back, int G_back, int B_back)
{
	...
	//Set Window
    disp_setwindow(disp, start_x, start_y, start_x+width - 1, start_y+height - 1);
    
    uint8_t send_data[width * height * 2];
	...
	for(int h = 0; h < height; h++)
	{
        ...
        if (let == 248)
        {
            letter = SmallFont[95*12 + 4 + h];
        }
        else
        {
            letter = SmallFont[(let- 0x20)*12 + 4 + h];
        }
		...
		for (int w = 0; w < width; w++)
		{
			if(binary[w] == 1)
			{
                uint16_t color = RGB(R_char, G_char, B_char);
			...
			{
                uint16_t color = RGB(R_back, G_back, B_back);
...
void w_text(spi_display_t *disp, char* txt, int start_x, int start_y, int R_char, int G_char, int B_char, int R_back, int G_back, int B_back)
{
	...
	for(int i = 0; txt[i] != '\0'; i++)
	{
		...
		w_letter(disp, txt[i], x, y,font_height,font_width, R_char, G_char, B_char, R_back, G_back, B_back);
		x = x + font_width;
	}
}
